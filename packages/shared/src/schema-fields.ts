...
export const projectMtlsEnabledSchema = yupBoolean().meta({ openapiField: { description: 'Whether mTLS (client certificate) authentication is enabled for this project', exampleValue: true } });
export const projectAnonymousSignInEnabledSchema = yupBoolean().meta({ openapiField: { description: 'Whether visitors can start an anonymous "guest" session for this project', exampleValue: true } });
...
export const oauthScopeSchema = yupString().meta({ openapiField: { description: 'Space-separated OAuth scopes to request from a custom SSO provider. Defaults to "openid email profile" if not specified.', exampleValue: 'openid email profile' } });
export const oauthProviderDisplayNameSchema = yupString().meta({ openapiField: { description: 'Human-readable name shown on the sign-in button for a custom SSO provider.', exampleValue: 'My Identity Provider' } });
export const oauthProviderIconUrlSchema = yupString().meta({ openapiField: { description: 'Icon shown on the sign-in button for a custom SSO provider. Either an absolute image URL or a data URI. Only used for custom SSO providers.', exampleValue: 'https://example.com/icon.png' } });
export const oauthAuthorizationEndpointSchema = urlSchema.meta({ openapiField: { description: 'OAuth 2.0 authorization endpoint. Required for custom OAuth 2.0 providers (type "custom_oauth").', exampleValue: 'https://your-idp.example.com/oauth/authorize' } });
export const oauthTokenEndpointSchema = urlSchema.meta({ openapiField: { description: 'OAuth 2.0 token endpoint. Required for custom OAuth 2.0 providers (type "custom_oauth").', exampleValue: 'https://your-idp.example.com/oauth/token' } });
export const oauthUserinfoEndpointSchema = urlSchema.meta({ openapiField: { description: 'OAuth 2.0 userinfo endpoint used to fetch the user profile. Required for custom OAuth 2.0 providers (type "custom_oauth").', exampleValue: 'https://your-idp.example.com/oauth/userinfo' } });
export const oauthTrustEmailVerifiedSchema = yupBoolean().meta({ openapiField: { description: 'When true, emails from this custom SSO provider are treated as verified even if the provider does not return an "email_verified" claim. Only enable for trusted providers, since it affects same-email account linking.', exampleValue: false } });
