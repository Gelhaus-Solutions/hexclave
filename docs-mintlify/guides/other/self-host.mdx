...
The dashboard's Managed Domain email flow is an operator-managed integration. It requires additional server-side provider credentials such as `STACK_RESEND_API_KEY`, `STACK_DNSIMPLE_API_TOKEN`, and `STACK_DNSIMPLE_ACCOUNT_ID`. See [Managed Email Setup](/guides/other/managed-email) for the full operator guide. If you do not operate that integration, use Custom SMTP or your own Resend API key instead.
